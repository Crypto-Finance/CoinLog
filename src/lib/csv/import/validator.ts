 ...
 * Trims whitespace and strips leading formula-injection characters (=, +, @, -).
 * Preserves negative numbers (minus followed by digit).
 * 
 * Security: Prevents CSV injection attacks where formulas like =CMD(...) or -=CMD(...)
 * could be executed by Excel/Sheets. See OWASP CSV Injection.
 */
export function sanitizeCSVField(value: string): string {
  let result = value.trim();
  // Repeatedly strip leading formula characters.
  // - Strip =, +, @ unconditionally (always formula starters)
  // - Strip - only when followed by =, +, @, or a letter (formula pattern)
  // - Preserve - when followed by a digit or $ (legitimate negative number)
  //
  // e.g., "=-CMD" → strip "=" → "-CMD" → strip "-" (followed by letter) → "CMD"
  // e.g., "=-100.50" → strip "=" → "-100.50" → stop (- followed by digit)
  // e.g., "-$100" → stop (- followed by $)
  // e.g., "-=CMD" → strip "-" (followed by =) → "=CMD" → strip "=" → "CMD"
  while (true) {
    const prev = result;
    // Strip leading =, +, @
    result = result.replace(/^[=+@]+/, '');
    // Strip leading - if followed by =, +, @, or letter (formula pattern)
 ...
 * @throws if direction is not 'Long' or 'Short'
 */
function validateDirection(getField: FieldAccessor, rowIdx: number): void {
 ...
 * @throws if numeric fields are invalid or out of range
 */
function validateRequiredNumericFields(
  getField: FieldAccessor,
  rowIdx: number,
): void {
  const fields: { name: string; min?: number; positive?: boolean }[] = [
  ...
  for (const { name, min, positive } of fields) {
    const raw = getField(name);
    if (!raw) continue;
    const num = parseFloat(raw);
    if (isNaN(num)) {
      throw new Error(`Row ${rowIdx + 2}: "${name}" must be a valid number, got "${raw}"`);
    }
    if (min !== undefined && num < min) {
      throw new Error(`Row ${rowIdx + 2}: "${name}" must be >= ${min}, got ${num}`);
    }
    if (positive && num <= 0) {
      throw new Error(`Row ${rowIdx + 2}: "${name}" must be positive, got ${num}`);
    }
  }
}
 ...
 * @throws if optional numeric fields are present but invalid
 */
...
export function validateRow(
  ...
  validateDirection(getField, rowIdx);
  validateRequiredNumericFields(getField, rowIdx);
  validateOptionalNumericFields(getField, rowIdx);
}
